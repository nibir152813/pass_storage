...
import { passwordAPI, getUser } from "../utils/api";
...
const Manager = () => {
  ...
  const user = getUser();
  const MASTER_PASSWORD = user?.email || "master";
  ...
  const handleDialogSubmit = () => {
    // For now, we can use a simple verification or remove it
    // Since we have JWT auth, master password is optional
    if (dialogPassword === MASTER_PASSWORD || dialogPassword.length > 0) {
      ...
      toast.error("Please enter a verification password!", {
        position: "top-right",
        autoClose: 3000,
        theme: "dark",
      });
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-8 w-[90%] max-w-md transform transition-all animate-fadeIn">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-gray-800">
                {isVerified ? "Password Details" : "Verify to View Password"}
              </h2>
              <button
            ...
            {!isVerified ? (
              <>
                <p className="text-gray-600 mb-6">
                  Enter any password to view this saved password.
                </p>
